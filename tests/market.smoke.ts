...
async function main() {
  ...
  const buy = await buyAsset({ gameId: game.id, playerId: player.id, symbol: "GOLD", quantity: 10 });
  ...
  const sell = await sellAsset({ gameId: game.id, playerId: player.id, symbol: "GOLD", quantity: 5 });
